...
for g in switch_games:
    ...
    # Require BOTH substring lengths >= 5 to prevent short title collisions!
    if score == 0.0:
        for m_name, m_score in mc_map.items():
            if len(n_title) >= 5 and len(m_name) >= 5 and (n_title == m_name or n_title in m_name or m_name in n_title):
                score = m_score
                break
...
def is_arcade(item):
    ...
    if any(c in cats for c in ["Fighting", "Shooter", "First-Person Shooter", "Файтинг", "Шутер", "Шутер от первого лица"]):
        return False
    if any(w in sw for w in ["fighting", "shooter", "fps", "shmup", "beatemup"]):
        return False
...
print("\n=== PERFECT METACRITIC COLLECTIONS CREATED ===")
