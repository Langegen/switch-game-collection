...
# Process switch_games.json items
valid_db = []
...
for g in switch_games:
    ...
    # Metacritic score lookup
    score = mc_map.get(n_title, 0.0)
    # Check partial title match in mc_map if not exact
    if score == 0.0:
    ...
    # Fallback heuristic score based on year/publisher if metacritic missing
    if score == 0.0:
        year_m = re.search(r"20\d\d", str(g.get("year", "")))
        yr = int(year_m.group(0)) if year_m else 2018
        pub = str(g.get("publisher", "")).lower()
        score = 65.0 + (yr - 2017) * 0.5 + (10.0 if "nintendo" in pub else 0.0)
        
    # Categories from TitleDB + switch_games.json genre field
...
# Strict Single-Genre Filters
def is_arcade(item):
    # Strictly Arcade (Аркада / Arcade), NO Fighting, NO Shooter
    cats = item["tdb_cats"]
    sw = item["sw_genre"]
    
    # Exclude fighting & shooters
    if any(c in cats for c in ["Fighting", "Shooter", "First-Person Shooter", "Файтинг", "Шутер", "Шутер от первого лица"]):
        return False
    if "fighting" in sw or "shooter" in sw or "fps" in sw or "beatemup" in sw or "shmup" in sw:
        return False
        
    if "Arcade" in cats or "Аркада" in cats or "arcade" in sw:
        return True
    return False
...
def is_action_adventure(item):
    ...
    # Exclude pure platformers and visual novels
...
def is_horror(item):
    ...
    return "horror" in sw or "хоррор" in sw or "survival horror" in desc or "ужас" in desc

def is_metroidvania(item):
    sw = item["sw_genre"]
    desc = str(item["game"].get("description", "")).lower()
    t = item["clean_title"].lower()
    metroidvanias = ["hollow knight", "metroid", "ori ", "ori and", "blasphemous", "bloodstained", "axiom verge", "ender lilies", "ender magnolia", "guacamelee", "castlevania", "steamworld dig", "the messenger", "monster boy", "wonder boy", "salt and sanctuary", "timespinner", "chasm", "islets", "yoku", "nine sols", "minoria", "afterimage", "haiku", "infernax"]
...
def is_visual_novel(item):
    ...
    desc = str(item["game"].get("description", "")).lower()
    t = item["clean_title"].lower()
    vns = ["ace attorney", "danganronpa", "steins", "ai: the somnium", "va-11", "coffee talk", "13 sentinels", "fata morgana", "raging loop", "clannad", "cupid parasite", "collar x malice", "code: realize", "bustafellows", "rain code", "paranormasight", "world end syndrome", "tsukihime", "chaos", "robotics", "anonymous code", "kanon", "doki doki"]
...
for filepath in sorted(collection_files):
    ...
    # If candidates were fewer than 100, fill remaining with highest rated available games
    if len(selected) < 100:
    ...
    # Write back clean JSON (1 item per line)
    lines = ["[\n"]
...
print("\n=== METACRITIC COLLECTIONS CREATED ===")
